...
| [Retrieval safety and evaluation](docs/knowledge/rag-golden-dataset.md) | Injection filtering runs before ranking; a versioned Dataset and isolated report exercise fact, exact-ID, multi-source, no-answer, leakage, and injection cases through deterministic or explicitly authorized semantic embedding profiles | [runner](apps/api/internal/rageval/runner.go), [evaluation](apps/api/internal/rag/evaluation.go), [dataset](examples/knowledge/golden-dataset.v1.json) |
