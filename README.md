...
| [Isolated Worker Stage](docs/runtime/isolated-worker-stage.md) | Multi-Agent Workers execute inside the parent Run with isolated context, frozen Tool authority, bounded result handoff, cancellation, checkpoints, and one replayable lifecycle | [runtime](apps/api/internal/agent/worker_stage.go), [tests](apps/api/internal/agent/orchestrator_test.go) |
...
| [Agent routing evaluation](docs/evaluation/offline-evaluation.md#agent-routing-gate) | A versioned calibration/holdout dataset gates the single Agent selection algorithm; an immutable Artifact preserves the retired algorithm comparison without keeping old code executable | [runner](apps/api/internal/evaluation/routeeval/runner.go), [dataset](examples/routing/golden-dataset.v1.json), [retirement evidence](examples/routing/legacy-algorithm-baselines.json) |
...
| [Provider compatibility](docs/architecture/engineering-decisions.md#openai-compatible-does-not-mean-capability-identical) | The native Turn Engine requires structured Tool Calling and reports unsupported models explicitly; stream usage metadata has a bounded transport fallback | [provider adapter](apps/api/internal/inference/openai), [tests](apps/api/internal/inference/openai/context_integration_test.go) |
...
| [Retrieval safety and evaluation](docs/evaluation/rag-golden-dataset.md) | Injection filtering runs before ranking; a versioned Dataset and isolated report exercise fact, exact-ID, multi-source, no-answer, leakage, and injection cases through deterministic or explicitly authorized semantic embedding profiles | [runner](apps/api/internal/evaluation/rageval/runner.go), [evaluation](apps/api/internal/rag/evaluation.go), [dataset](examples/knowledge/golden-dataset.v1.json) |
...
| [**Bounded load evidence**](docs/evaluation/load-soak-testing.md) | A controlled five-segment load profile reports saturation, latency, rejection, cancellation, and cleanup without treating stub throughput as model capacity | [harness](apps/api/internal/evaluation/loadtest/load_soak_test.go), [evidence command](scripts/evaluation/load-evidence.sh) |
| [**Release and recovery drill**](docs/operations/release-recovery-drill.md) | A real single-instance API process is drained, killed during a checkpointed Stage, restarted, repaired, resumed, and protected from duplicate Resume against disposable PostgreSQL | [drill](apps/api/internal/evaluation/releasedrill/release_drill_test.go), [command](scripts/release-recovery-drill.sh) |
...
| [Tool governance](docs/tools/tool-security-policy.md) | Platform enablement, Agent allowlists, frozen capability scope, operator policy, Budget, side-effect recovery, and concurrency remain explicit independent controls | [policy](apps/api/internal/tool/policy/policy.go), [executor](apps/api/internal/tool/executor.go), [tests](apps/api/internal/tool/security_executor_test.go) |
| [Tool Progress Guard](docs/tools/tool-progress-guard.md) | Run-scoped repeated failure/result detection escalates from warning to pre-Budget blocking and Turn halt with replayable evidence | [guard](apps/api/internal/tool/progress/guard.go), [tests](apps/api/internal/tool/progress/guard_test.go) |
| [Tool side-effect reconciliation](docs/tools/tool-side-effect-reconciliation.md) | Versioned operator commands resolve uncertain external writes with capability-gated retry/compensation and typed audit events | [coordinator](apps/api/internal/tool/reconciliation/reconciliation.go), [tests](apps/api/internal/tool/reconciliation/reconciliation_test.go) |
...
| [**Evidence comparison**](docs/evaluation/evidence-comparison.md) | The Evaluation workspace compares two controlled, Workspace-scoped Runs only when task, materials, and runtime identity permit meaningful deltas | [view](apps/web/components/evaluation/EvidenceComparison.tsx), [comparison rules](apps/web/lib/evidence-comparison.ts) |
| [**Evidence-backed benchmark suite**](docs/evaluation/offline-evaluation.md#case-001a-live-model-benchmark-evidence) | A fixed 12-task manifest connects deterministic regression evidence with an opt-in, budgeted live RAG/Tool/Router comparison pack | [manifest](examples/benchmark-suite.v1.json), [offline evidence](scripts/evaluation/benchmark-evidence.sh), [live CLI](apps/api/cmd/eval/live_benchmark.go) |
...
5. Run `make benchmark-evidence`, then `make demo-check`, to validate the
   no-network CASE-001 and recovery backup before starting the demo. Run
