...
RAG search accepts an optional positive `context_token_budget`. The response
...
The top-level `context_selection` object contains `version`, `token_budget`,
...
respective sizes, while `context_selection` records the budget decision.
