...
func TestUpdateTaskStateToolAppliesPatchAndPublishesEvent(t *testing.T) {
	...
	conversation, _ := fixtureStore.CreateConversation("tool test")
	run, err := fixtureStore.CreateRunWithContract("agent_planner", conversation.ID, taskStateTestSnapshot(), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	state, ok, err := fixtureStore.GetTaskState(conversation.ID)
	...
	revisions, err := fixtureStore.ListTaskStateRevisions(conversation.ID)
