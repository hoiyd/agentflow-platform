...
func (c *Client) streamOpenAIWithTools(ctx context.Context, systemPrompt string, history []domain.Message, latest string, catalog *tool.Catalog, events chan<- StreamEvent, recorder *tracepkg.Recorder, runID string, stepID string, retrievedMemories []domain.RetrievedMemory, retrievedChunks []domain.RetrievedDocumentChunk) error {
	...
	toolExecutor := tool.NewExecutor(catalog, tool.ExecutorOptions{
		...
		ProgressGuard:        progress.FromContext(ctx),
	})
	scope := tracepkg.ScopeFromContext(ctx)
	requests := make([]tool.ExecutionRequest, 0, len(normalizedCalls))
