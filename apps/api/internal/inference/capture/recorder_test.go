...
func TestRecorderFailsClosedAndSkipsUnscopedCalls(t *testing.T) {
	...
	validObservation := requestcontrol.Observation{
	...
	if err := NewRecorder(&captureStoreStub{run: validRun, ok: true}, Options{}).Record(scoped, requestcontrol.Observation{}); err == nil {
	...
	tests := []struct {
		...
		{name: "invalid payload", store: &captureStoreStub{run: validRun, ok: true}, observation: requestcontrol.Observation{
