...
func TestRequiredWorkspaceRejectsUnscopedAPIRequests(t *testing.T) {
	fileStore, err := store.NewFileStore(t.TempDir() + "/agentflow.json")
	if err != nil {
		t.Fatalf("new store: %v", err)
	}
	handler := &Handler{store: fileStore, workspace: WorkspacePolicy{DefaultID: "default", Required: true}}

	request := httptest.NewRequest(http.MethodGet, "/api/conversations", nil)
	recorder := httptest.NewRecorder()
	handler.Routes().ServeHTTP(recorder, request)
	if recorder.Code != http.StatusBadRequest {
		t.Fatalf("expected missing workspace to return 400, got %d body=%s", recorder.Code, recorder.Body.String())
	}
...
func TestExplicitWorkspaceRejectsMismatchedPayload(t *testing.T) {
	...
	request.Header.Set(WorkspaceHeader, "workspace-a")
	handler := &Handler{workspace: WorkspacePolicy{Required: true}}
	recorder := httptest.NewRecorder()
