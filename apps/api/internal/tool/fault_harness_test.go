...
func TestEffectGateExposesIntentEffectAndSettlementBoundaries(t *testing.T) {
	...
	executor := tool.NewExecutor(catalog, tool.ExecutorOptions{EffectJournal: fixture})
	resultCh := make(chan tool.ExecutionResult, 1)
	go func() {
		resultCh <- executor.Execute(context.Background(), tool.ExecutionRequest{
...
func TestEffectGateInjectsJournalFailuresWithoutNetwork(t *testing.T) {
	t.Run("intent failure", func(t *testing.T) {
		...
		tooltest.AssertTypedFailure(t, result, tool.ErrorEffectJournal)
...
func executeEffectFixture(t *testing.T, fixture *tooltest.EffectGateFixture) tool.ExecutionResult {
	t.Helper()
	catalog, _, err := tooltest.NewAuthorizedCatalog(fixture.Binding("write_record"))
	if err != nil {
		t.Fatalf("new catalog: %v", err)
