...
func TestToolSelectionGoldenDatasetMatchesCurrentContracts(t *testing.T) {
	...
	defaultCatalog := tool.DefaultCatalog()
	...
	for _, outcome := range []string{"no_tool", "success", string(tool.ErrorInvalidArgs), string(tool.ErrorExecutionFailed)} {
...
func selectionCatalog(t *testing.T) *tool.Catalog {
	t.Helper()
	catalog, err := tool.DefaultCatalog().CloneWith((&taskstate.Service{}).ToolBinding())
	if err != nil {
...
func executeGoldenCall(catalog *tool.Catalog, item tooltest.SelectionCase) string {
	result := tool.NewExecutor(catalog, tool.ExecutorOptions{}).Execute(context.Background(), tool.ExecutionRequest{
