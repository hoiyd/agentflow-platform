...
func TestArtifactToolsReadSearchAndTraceWithinRun(t *testing.T) {
	...
	catalog, err := tool.NewCatalog(service.ToolBindings()...)
	if err != nil {
		t.Fatal(err)
	}
	...
	read := executor.Execute(ctx, tool.ExecutionRequest{
	...
	search := executor.Execute(ctx, tool.ExecutionRequest{
...
func TestArtifactToolRecordsExpiredAccess(t *testing.T) {
	...
	service := NewService(fixtureStore, eventpkg.NewRecorder(fixtureStore))
	catalog, _ := tool.NewCatalog(service.ToolBindings()...)
	ctx := eventpkg.WithScope(context.Background(), eventpkg.Scope{RunID: run.ID, ConversationID: run.ConversationID})
	result := tool.NewExecutor(catalog, tool.ExecutorOptions{}).Execute(ctx, tool.ExecutionRequest{
...
func TestCuratedArtifactBindingsRejectScopeAndPolicyAndBoundResults(t *testing.T) {
	...
	for _, binding := range service.ToolBindings() {
		t.Run(binding.Descriptor.Name, func(t *testing.T) {
			...
			call := tool.ExecutionRequest{CallID: "scope", RunID: "other-run", Tool: binding.Descriptor.Name, Arguments: args}
			tooltest.AssertTypedFailure(t, executor.Execute(ctx, call), tool.ErrorExecutionFailed)
			denied, err := tool.NewCatalogWithPolicy(policy.Policy{Version: policy.CurrentVersion, DefaultAction: policy.ActionDeny}, binding)
			if err != nil {
				t.Fatal(err)
			}
			ctx = eventpkg.WithScope(context.Background(), eventpkg.Scope{RunID: run.ID})
			call.RunID = run.ID
			tooltest.AssertTypedFailure(t, tool.NewExecutor(denied, tool.ExecutorOptions{}).Execute(ctx, call), tool.ErrorSecurityPolicyDenied)
			binding.Policy.MaxResultBytes = 32
			bounded, err := tool.NewCatalog(binding)
			if err != nil {
				t.Fatal(err)
			}
			result := tool.NewExecutor(bounded, tool.ExecutorOptions{}).Execute(ctx, call)
