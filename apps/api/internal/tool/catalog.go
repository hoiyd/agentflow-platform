...
func NewCatalog(bindings ...Binding) (*Catalog, error) {
	return NewCatalogWithPolicy(policy.DefaultPolicy(), bindings...)
}

func NewCatalogWithPolicy(securityPolicy policy.Policy, bindings ...Binding) (*Catalog, error) {
	securityPolicy = policy.NormalizePolicy(securityPolicy)
	if err := policy.ValidatePolicy(securityPolicy); err != nil {
...
func (c *Catalog) Register(binding Binding) error {
	...
	binding.Descriptor.Security = policy.NormalizeCapability(binding.Descriptor.Security)
	...
	if binding.Descriptor.SideEffect.Compensate && binding.Descriptor.Security.Reversibility == policy.Irreversible {
...
func (c *Catalog) SecurityPolicy() policy.Policy {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return policy.NormalizePolicy(c.securityPolicy)
}
