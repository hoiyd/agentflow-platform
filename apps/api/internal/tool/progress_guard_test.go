...
func (t *progressTrace) ToolProgressEvaluated(_ context.Context, _ ExecutionRequest, decision progress.Decision) {
...
func TestExecutorProgressGuardWarnsBlocksAndHaltsRepeatedFailure(t *testing.T) {
	...
	trace := &progressTrace{}
	executor := NewExecutor(catalog, ExecutorOptions{ProgressGuard: progress.New(progress.DefaultConfig()), Tracer: trace})
	...
	for attempt := 1; attempt <= 5; attempt++ {
		...
		case 4:
			if result.Error == nil || result.Error.Code != ErrorProgressBlocked || result.ProgressDecision.Action != progress.ActionBlockCall {
				t.Fatalf("blocked result=%#v", result)
			}
		case 5:
			if result.Error == nil || result.Error.Code != ErrorNoProgress || result.ProgressDecision.Action != progress.ActionHaltTurn {
	...
	if len(trace.decisions) != 4 || trace.decisions[0].Action != progress.ActionWarn || trace.decisions[len(trace.decisions)-1].Action != progress.ActionHaltTurn {
...
func TestExecutorProgressGuardTracksReadOnlyOnlyAndNormalizesFallbackArguments(t *testing.T) {
	...
	executor := NewExecutor(catalog, ExecutorOptions{ProgressGuard: progress.New(progress.DefaultConfig())})
	for range 6 {
