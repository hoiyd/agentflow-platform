...
func NewToolEffectViews(catalog *tool.Catalog, records []domain.ToolEffectRecord) []ToolEffectView {
...
func ReconcileToolEffect(ctx context.Context, catalog *tool.Catalog, target effectReconciliationStore, run domain.Run, idempotencyKey string, command ToolEffectReconciliationCommand) (ToolEffectReconciliationOutcome, error) {
	...
	if external {
		...
		if timeout <= 0 || timeout > tool.DefaultExecutionTimeout {
			timeout = tool.DefaultExecutionTimeout
		}
...
func executeReconciliationAction(ctx context.Context, catalog *tool.Catalog, record domain.ToolEffectRecord, command ToolEffectReconciliationCommand) (domain.ToolEffectStatus, []byte, error) {
	...
	case domain.ToolEffectRetrySameKey, domain.ToolEffectCompensate:
		...
		recovery := tool.EffectReconciliationContext{
			CommandID: command.CommandID, IdempotencyKey: record.IdempotencyKey,
		...
		if !binding.Descriptor.SideEffect.Compensate || binding.Reconciliation.Compensate == nil || binding.Descriptor.Security.Reversibility == policy.Irreversible {
...
func reconciliationBinding(catalog *tool.Catalog, record domain.ToolEffectRecord) (tool.Binding, error) {
	if catalog == nil {
	...
	if record.DefinitionRevision == "" || binding.Descriptor.DefinitionRevision != record.DefinitionRevision {
		return tool.Binding{}, &ReconciliationError{Code: ReconciliationMismatch, Message: "tool definition revision does not match the uncertain effect"}
	}
	return binding, nil
}

func availableReconciliationActions(catalog *tool.Catalog, effect domain.ToolEffectSummary) []domain.ToolEffectReconciliationAction {
...
func reconciliationCommandOutcome(target effectReconciliationStore, catalog *tool.Catalog, runID string, record domain.ToolEffectRecord, commandID, commandHash string) (*ToolEffectReconciliationOutcome, error) {
...
func validateReconciliationCommand(idempotencyKey string, command ToolEffectReconciliationCommand) error {
	...
	if command.Action == domain.ToolEffectConfirmCommitted {
		if len(command.Result) == 0 || len(command.Result) > tool.DefaultMaxResultBytes || !json.Valid(command.Result) {
...
func encodeReconciledResult(record domain.ToolEffectRecord, value any) ([]byte, error) {
	encoded, err := json.Marshal(tool.ExecutionResult{
		CallID: record.ToolCallID, Tool: record.ToolName, Result: value,
		DefinitionRevision: record.DefinitionRevision,
	})
	if err != nil || len(encoded) > tool.DefaultMaxResultBytes {
		return nil, fmt.Errorf("reconciled Tool result is invalid or exceeds %d bytes", tool.DefaultMaxResultBytes)
	}
...
func callEffectRetry(ctx context.Context, callback func(context.Context, tool.EffectReconciliationContext) (any, error), recovery tool.EffectReconciliationContext) (result any, err error) {
	defer func() {
...
func callEffectCompensation(ctx context.Context, callback func(context.Context, tool.EffectReconciliationContext) error, recovery tool.EffectReconciliationContext) (err error) {
	defer func() {
		if recovered := recover(); recovered != nil {
