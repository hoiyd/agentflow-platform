...
func (m runtimeTurnModel) Execute(ctx context.Context, request turn.Request, emit func(turn.ModelEvent)) (turn.Result, error) {
	...
	for events != nil || errs != nil {
		...
		case event, ok := <-events:
			...
			case "tool_end":
				eventType := turn.EventToolFinished
				if event.Error != "" {
					eventType = turn.EventToolFailed
				}
				emit(turn.ModelEvent{Type: eventType, ToolName: event.ToolName, ToolCallID: event.ToolCallID, Error: event.Error})
			}
		case err, ok := <-errs:
			if !ok {
				errs = nil
